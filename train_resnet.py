...
class Sub_model(nn.Module):
    def __init__(self, args):
        ...
        self.block4 = PreActBottleneck(cin=3072, cout=3072, cmid=768, stride=2)

        self.head = nn.Sequential(OrderedDict([
            ('gn', nn.GroupNorm(32, 3072)),
            ('relu', nn.ReLU(inplace=True)),
            ('avg', nn.AdaptiveAvgPool2d(output_size=1)),
            ('conv', nn.Conv2d(3072, 10, kernel_size=1, bias=True)),
        ]))

    def forward(self, x):
        x = self.head(self.block4(self.block3(self.block2(self.block1(self.root(x))))))
...
def train(args, model):
    ...
    if args.local_rank in [-1, 0]:
        ...
        writer = SummaryWriter(log_dir=os.path.join("logs_resnet", args.name))
    ...
    train_metric.to_csv('train_metric_params.csv', index=False)
    test_metric.to_csv('test_metric_params.csv', index=False)
...
def main():
    ...
    parser.add_argument("--output_dir", default="output_params", type=str,
